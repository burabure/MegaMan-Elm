...
Elm.Entity = Elm.Entity || {};
Elm.Entity.make = function (_elm) {
   "use strict";
   _elm.Entity = _elm.Entity || {};
   if (_elm.Entity.values) return _elm.Entity.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var colliding = function (_p0) {
      var _p1 = _p0;
      var _p3 = _p1._0;
      var _p2 = _p1._1;
      var rightOf = function (ent) {    return ent.x + ent.w / 2;};
      var focusDiffRight = rightOf(_p3) - rightOf(_p2);
      var leftOf = function (ent) {    return ent.x - ent.w / 2;};
      var focusDiffLeft = leftOf(_p3) - leftOf(_p2);
      return _U.cmp(focusDiffLeft,0) < 0 && _U.cmp(focusDiffRight,
      0) > 0 ? $Maybe.Just(Center) : _U.cmp(focusDiffLeft - _p2.w,
      0) < 1 && _U.cmp(focusDiffLeft,
      0) > -1 ? $Maybe.Just(Left) : _U.cmp(focusDiffRight + _p2.w,
      0) > -1 && _U.cmp(focusDiffRight,
      0) < 1 ? $Maybe.Just(Right) : $Maybe.Nothing;
   };
   var Entity = F5(function (a,b,c,d,e) {
      return {id: a,x: b,y: c,w: d,h: e};
   });
   var toEntity = function (record) {
      return A5(Entity,
      record.id,
      record.x,
      record.y,
   ...
   return _elm.Entity.values = {_op: _op
                               ,Entity: Entity
                               ,toEntity: toEntity
                               ,Right: Right
                               ,Left: Left
...
Elm.Player.make = function (_elm) {
   ...
   var view = function (model) {
      ...
      "./sprites/player/",
      A2($Basics._op["++"],direction,".gif"));
      return A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: model.x,_1: model.y},
      $Graphics$Collage.toForm(A3($Graphics$Element.image,
      $Basics.truncate(model.w),
      $Basics.truncate(model.h),
      spriteSrc)));
   };
   var MoveX = function (a) {    return {ctor: "MoveX",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var Right = {ctor: "Right"};
   var Left = {ctor: "Left"};
   var update = F2(function (action,player) {
      var _p1 = action;
      if (_p1.ctor === "NoOp") {
            return player;
         } else {
            var _p2 = _p1._0;
            var newDir = _U.cmp(_p2,0) < 0 ? Left : _U.cmp(_p2,
            0) > 0 ? Right : player.dir;
            var units = $Basics.toFloat(_p2 * 5);
            return _U.update(player,{x: player.x + units,dir: newDir});
         }
   });
   var Model = F6(function (a,b,c,d,e,f) {
      return {id: a,x: b,y: c,w: d,h: e,dir: f};
   });
...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var log = F2(function (title,stringable) {
   ...
   var update = F2(function (action,model) {
      ...
      {player: A2($Player.update,_p0._0,model.player)});
   });
   var PlayerAction = function (a) {
      return {ctor: "PlayerAction",_0: a};
   };
   var Model = function (a) {    return {player: a};};
   var directions = A2($Signal.map,
   function (_p1) {
      return PlayerAction($Player.MoveX(function (_) {
         return _.x;
      }(_p1)));
   },
   A2($Signal.sampleOn,$Time.fps(60),$Keyboard.arrows));
   var boxEntity = A5($Entity.Entity,"box",90,-90,30,60);
   var box = A2($Graphics$Collage.move,
   {ctor: "_Tuple2",_0: boxEntity.x,_1: boxEntity.y},
   A2($Graphics$Collage.filled,
   A3($Color.rgb,75,75,75),
   A2($Graphics$Collage.rect,boxEntity.w,boxEntity.h)));
   var stage = {background: A2($Graphics$Collage.filled,
               A3($Color.rgb,230,230,255),
               A2($Graphics$Collage.rect,500,250))
               ,playerStartingPoint: {ctor: "_Tuple2",_0: 0,_1: -90}};
   ...
   var models = A3($Signal.foldp,update,initialModel,directions);
   var view = function (model) {
      var playerCollision = $Entity.colliding({ctor: "_Tuple2"
                                              ,_0: $Entity.toEntity(model.player)
                                              ,_1: boxEntity});
      return A3($Graphics$Collage.collage,
      500,
      250,
      _U.list([stage.background
              ,$Player.view(model.player)
              ,box
              ,A2(log,"Collision",playerCollision)]));
   };
   var main = A2($Signal.map,view,models);
   var Stage = F2(function (a,b) {
      return {background: a,playerStartingPoint: b};
   });
